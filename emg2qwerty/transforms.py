...
class RandomShift:
    ...
    def __call__(self, tensor: torch.Tensor) -> torch.Tensor:
        ...
        return tensor.roll(shift, dims=self.time_dim)
@dataclass
class AddNoise:
    ...
    def __call__(self, tensor: torch.Tensor) -> torch.Tensor:
        ...
        # Add the noise to the original tensor and return
        return tensor + noise
# @dataclass
# class RandomCrop:
#     crop_size: int
...
#         # If the tensor is shorter than crop_size, return it unchanged.
#         if T <= self.crop_size:
#             return tensor
#         # Randomly choose a start index such that the crop fits within the tensor.
#         start = np.random.randint(0, T - self.crop_size + 1)
#         # Create slicing tuple for all dimensions.
#         slices = [slice(None)] * tensor.ndim
#         slices[self.time_dim] = slice(start, start + self.crop_size)
